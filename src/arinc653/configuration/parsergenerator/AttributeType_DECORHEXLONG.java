...
/**
 * Attribute type - DECORHEXLONG.
 * 
 * @author Luís Fernando Arcaro
 */
public class AttributeType_DECORHEXLONG extends AttributeType {

	/**
	 * Instance.
	 */
	public static final AttributeType_DECORHEXLONG INSTANCE = new AttributeType_DECORHEXLONG();

	/**
	 * Constructor.
	 */
	private AttributeType_DECORHEXLONG() {
		super("DecOrHexLong", "dh");
	}
	...
	public void cmdAddImports(ParserGenerator pgParserGenerator, Set<String> stImport) {
		...
		// Adds imports
		stImport.add(pgParserGenerator.stResourcesPackage + ".DecOrHexLong");
	}
	...
	public void cmdGenerateLoad(ParserGenerator pgParserGenerator, String stIndentation, Node ndSubject, List<String> lsClass, Set<String> stImport, Attribute atAttribute) {
		...
		lsClass.add(stIndentation + "		nd" + ndSubject.getJavaName() + "." + atAttribute.getAttributeType().getJavaPrefix() + atAttribute.getJavaName() + " = ConfigurationUtilities.getDecOrHexLongAttribute(ndNode, " + ndRoot.getJavaName() + ".ATTRIBUTENAME_" + atAttribute.getJavaName().toUpperCase() + ");");
	...
	public void cmdGenerateSave(ParserGenerator pgParserGenerator, String stIndentation, Node ndSubject, List<String> lsClass, Set<String> stImport, Attribute atAttribute) {
		...
		// Generates
		lsClass.add(stIndentation + "	el" + ndSubject.getJavaName() + ".setAttribute(" + ndRoot.getJavaName() + ".ATTRIBUTENAME_" + atAttribute.getJavaName().toUpperCase() + ", ConfigurationUtilities.getAttributeDecOrHexLong(nd" + ndSubject.getJavaName() + ".get" + atAttribute.getJavaName() + "()));");
	}
}